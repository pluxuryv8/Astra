// FROM computer-agent:src-tauri/src/computer.rs
...
// качество JPEG (1-100): меньше = быстрее и компактнее, 60 достаточно для скриншотов
...
pub struct ComputerAction {
    ...
    #[serde(default)]
    pub key: Option<String>,  // для действия hold_key
    #[serde(default)]
    pub region: Option<[i32; 4]>,  // для действия zoom [x1, y1, x2, y2]
}
...
impl ComputerControl {
    pub fn new() -> Result<Self, ComputerError> {
        let monitor = Monitor::all()
            ...
            .into_iter()
            .next()
            .ok_or_else(|| ComputerError::Screenshot("Монитор не найден".to_string()))?;
    ...
    pub fn take_screenshot(&self) -> Result<String, ComputerError> {
        ...
        // уменьшение с фильтром Nearest (самый быстрый) — достаточно для ИИ
        let resized = DynamicImage::ImageRgba8(image)
            .resize_exact(AI_WIDTH, AI_HEIGHT, FilterType::Nearest);

        // кодирование JPEG с контролем качества
        let rgb = resized.to_rgb8();
        let mut buffer = Vec::with_capacity(200_000); // предварительно ~200кб
    ...
    pub fn take_screenshot_excluding(&self, window_id: u32) -> Result<String, ComputerError> {
        ...
        // захват всех окон НИЖЕ нашего окна (исключая приложение и окна выше)
        ...
        // быстрый перевод BGRA -> RGB
        let mut rgb_data = Vec::with_capacity((width * height * 3) as usize);
        for y in 0..height {
            let row_start = (y * bytes_per_row) as usize;
            for x in 0..width {
                let offset = row_start + (x * 4) as usize;
                rgb_data.push(raw_data[offset + 2]); // красный
                rgb_data.push(raw_data[offset + 1]); // зелёный
                rgb_data.push(raw_data[offset]);     // синий
            }
        }

        let img = image::RgbImage::from_raw(width as u32, height as u32, rgb_data)
            .ok_or_else(|| ComputerError::Screenshot("Не удалось создать изображение".to_string()))?;
    ...
    pub fn perform_action(&self, action: &ComputerAction) -> Result<Option<String>, ComputerError> {
        ...
        // EN kept: названия действий и направлений — часть контракта навыка
        match action.action.as_str() {
            ...
            "scroll" => {
                ...
                // прокрутка enigo: положительное = вниз, отрицательное = вверх
                // горизонтальная: положительное = вправо, отрицательное = влево
            ...
            "hold_key" => {
                // hold_key удерживает модификатор при выполнении действия из поля text
                // например, удерживать Shift при клике
            ...
            "zoom" => {
                // zoom захватывает область в полном разрешении (без уменьшения)
                // region: [x1, y1, x2, y2] в координатах AI
                if let Some(region) = action.region {
                    let screenshot = self.take_screenshot_region(region)?;
                    Ok(Some(screenshot))
                } else {
                    // если область не указана — полный скриншот
                    let screenshot = self.take_screenshot()?;
                    Ok(Some(screenshot))
                }
            }
    ...
    fn type_text_applescript(&self, text: &str) -> Result<(), ComputerError> {
        ...
        // экранируем кавычки и слэши для AppleScript
        let escaped = text
            .replace('\\', "\\\\")
            .replace('"', "\\\"");

        // EN kept: системные вызовы AppleScript требуют английские идентификаторы
        let script = format!(
...
"#,
    ...
    fn press_key_cgevent(&self, key_str: &str) -> Result<(), ComputerError> {
        // разбор сочетаний клавиш (например, "cmd+t", "ctrl+shift+a")
        // EN kept: названия модификаторов в контракте навыка
        ...
        // сопоставляем названия клавиш с CGKeyCode (core_graphics::event::KeyCode)
        let key_code: CGKeyCode = match main_key.as_str() {
            ...
            "f11" => 0x67,
            "f12" => 0x6F,
            // одиночный символ — через таблицу
            _ => self.char_to_keycode(&main_key)?,
        };

        // создаём источник событий
        let source = CGEventSource::new(CGEventSourceStateID::HIDSystemState)
            .map_err(|_| ComputerError::Input("не удалось создать источник событий".to_string()))?;

        // нажатие клавиши
        let key_down = CGEvent::new_keyboard_event(source.clone(), key_code, true)
            .map_err(|_| ComputerError::Input("не удалось создать событие нажатия".to_string()))?;
        if !flags.is_empty() {
        ...
        let key_up = CGEvent::new_keyboard_event(source, key_code, false)
            .map_err(|_| ComputerError::Input("не удалось создать событие отпускания".to_string()))?;
        if !flags.is_empty() {
    ...
    pub fn take_screenshot_region_excluding(&self, region: [i32; 4], window_id: u32) -> Result<String, ComputerError> {
        ...
        // region: [x1, y1, x2, y2] в координатах AI, переводим в экранные
        ...
        // та же логика исключения, что и take_screenshot_excluding
        let options = kCGWindowListOptionOnScreenBelowWindow | kCGWindowListExcludeDesktopElements;
        ...
        let cg_image = unsafe {
            ...
            if img_ptr.is_null() {
                return Err(ComputerError::Screenshot("не удалось захватить область".to_string()));
            }
            core_graphics::image::CGImage::from_ptr(img_ptr)
        };
        ...
        // zoom возвращает полное разрешение, без уменьшения
        let mut buffer = Vec::with_capacity(200_000);
        let mut encoder = JpegEncoder::new_with_quality(&mut buffer, JPEG_QUALITY);
    ...
    /// запасной zoom без исключений (для не-macOS или когда window_id недоступен)
    ...
    fn hold_key_cgevent(&self, key_str: &str) -> Result<(), ComputerError> {
        // hold_key нажимает клавишу без отпускания
        // отпускание произойдёт следующим действием или по таймауту
        ...
        let flags = match key_lower.as_str() {
            ...
            _ => return Err(ComputerError::Input(format!("неизвестный модификатор: {}", key_str))),
        };

        let source = CGEventSource::new(CGEventSourceStateID::HIDSystemState)
            .map_err(|_| ComputerError::Input("не удалось создать источник событий".to_string()))?;

        // создаём событие с флагом модификатора, имитируя удержание
        let key_down = CGEvent::new_keyboard_event(source, 0, true)
            .map_err(|_| ComputerError::Input("не удалось создать событие клавиши".to_string()))?;
        key_down.set_flags(flags);
        key_down.post(CGEventTapLocation::HID);
    ...
    fn char_to_keycode(&self, c: &str) -> Result<CGKeyCode, ComputerError> {
        // сопоставляем одиночные символы с их кодами клавиш
        let code = match c.chars().next() {
            ...
            Some('/') => 0x2C,
            Some('`') => 0x32,
            _ => return Err(ComputerError::Input(format!("неизвестная клавиша: {}", c))),
        };
        Ok(code)
    }
        ...
        match key_lower.as_str() {
            ...
            _ => {
                if let Some(c) = key_str.chars().next() {
                    ...
                    Err(ComputerError::UnknownAction(format!("Неизвестная клавиша: {}", key_str)))
                }
            }
        }
    }
